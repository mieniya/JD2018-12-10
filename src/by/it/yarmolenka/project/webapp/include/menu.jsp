<nav class="navbar navbar-expand-lg navbar-light bg-light">
  <a class="navbar-brand" href="#">Navbar</a>
  <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
    <span class="navbar-toggler-icon"></span>
  </button>
  <div class="collapse navbar-collapse" id="navbarNav">
    <ul class="navbar-nav">
      <li class="nav-item">
        <a class="nav-link" href="do?command=Reset">Reset Data<span class="sr-only">(current)</span></a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="do?command=SignIn">Sign In</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="do?command=Register">Register</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="do?command=Logout">Logout</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="do?command=GetBets">My Bets</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="do?command=GetEvents">See Events</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="do?command=GetRaces">See Races</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="do?command=CreateEvent">Create Event</a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="do?command=CreateRace">Create Race</a>
      </li>
    </ul>
  </div>
</nav>
