...
<c:choose>
    <c:when test="${languageID.languageID==2}">
        <a class="navbar-brand" href=".">Платежная система TIT Co </a>
            ...
            <div class="navbar-nav">
                ...
                <c:choose>
                    <c:when test="${user.roles_id==1}">
                        <mytag:menu command="EditUsers" text="Список клиентов"/>
                        <mytag:menu command="EditAcc" text="Список счетов"/>
                    </c:when>
                    <c:when test="${user.roles_id==2}">
                        <mytag:menu command="CreateAcc" text="Открытие счета"/>
                        <mytag:menu command="AccTrans" text="Операции со счетами"/>
                        <mytag:menu command="AccInfo" text="Информация по счетам"/>
                    </c:when>
                    <c:otherwise>
            ...
            <ul class="navbar-nav flex-row ml-md-auto d-none d-md-flex">
                <c:choose>
                    ...
                    <c:otherwise>
                        ...
                        <mytag:menu command="Login" text="Вход"/>
                    </c:otherwise>
                </c:choose>
            </ul>
        </div>
    ...
    <c:otherwise>
        ...
        <div class="collapse navbar-collapse" id="navbarNavDropdown">
            <div class="navbar-nav">
                <a class="nav-item nav-link" href="do?command=ChangeLang">
                    <img src="images/globus.png" height="25px">EN
                </a>
                <c:choose>
                    <c:when test="${user.roles_id==1}">
