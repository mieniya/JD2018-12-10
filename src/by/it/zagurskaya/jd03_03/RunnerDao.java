...
import by.it.zagurskaya.jd03_03.beans.*;
import by.it.zagurskaya.jd03_03.dao.*;
...
//    ВАЖНО!!!! перед запуском запустить InitDataBase.java
    ...
    public static void main(String[] args) throws SQLException {
        ...
        OperationHasEntriesDao operationHasEntriesDao = new OperationHasEntriesDao();
        ...
        SprOperations sprOperations1 = testCRU(sprOperationsDao,
                new SprOperations(60,"Получено от клиентов","print60","screen60",""),
                r -> r.setName(r.getName()+"денежной наличности"));

        SprEntries sprEntries = testCRU(sprEntriesDao,
                new SprEntries(6001,"Получено денежной наличности (933)",840,null,"1010000000933",false,1),
                r -> r.setCurrencyId(933));

        OperationHasEntries operationHasEntries = testCRU(operationHasEntriesDao,
                new OperationHasEntries(0,sprOperations1.getId(),sprEntries.getId()),
                r -> r.setEntryId(1001));


        System.out.println();
        testD(operationHasEntriesDao, operationHasEntries);
