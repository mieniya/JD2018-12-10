...
public class Test_calc_many_expressions {

    @Test(timeout = 1500)
    public void testTaskA__ConsoleRunner() throws Exception {
        run("A=2+5.3\n" +
                "end\n")
                .include("7.3")    //A=2+5.3=7.3
                .exclude("ERROR:");

        run("B=A*3.5\n" +
                "end\n")
                .include("25.55")    //B=A*3.5=25.55
                .exclude("ERROR:");

        run("B1=B+0.11*-5\n" +
                "end\n")
                .include("25")    //B1=B+0.11*-5=25
                .exclude("ERROR:");
    ...
    public void testTaskB__ConsoleRunner() throws Exception {
        ...
        run("C=B+(A*2)\n" +
                "end\n")
                .include("40.15")    //C=B+(A*2)=40.15
                .exclude("ERROR:");

        run("D=((C-0.15)-20)/(7-5)\n" +
                "end\n")
                .include("10")    //D=((C-0.15)-20)/(7-5)=10
                .exclude("ERROR:");

        run("E={2,3}*(D/2)\n" +
                "end\n")
                .include("{10.0, 15.0}")    //E={2,3}*(D/2)={10,15}
                .exclude("ERROR:");
...
///!!!!!!!!!
        ...
        //!!!!!!!!!!
        run("AVector={1,1,1}+5.3\n" +
                "end\n")
                .include("{6.3, 6.3, 6.3}")    //{1,1,1}+5.3={6.3,6.3,6.3}
                .exclude("ERROR:");

        run("BVector=AVector*{1,2,3}\n" +
                "end\n")
                .include("37.8")    //BVector=AVector*{1,2,3}=37.8
                .exclude("ERROR:");

//        run("B1Vector=BVector+{1,1,1}*-5\n" +
//                "end\n")
//                .include("{32.8, 32.8, 32.8}")    //B1Vector=BVector+{1,1,1}*-5={32.8,32.8,32.8}
//                .exclude("ERROR:");


        run("B2Vector=AVector/2-1\n" +
                "end\n")
                .include("{2.15, 2.15, 2.15}")    //B2Vector=AVector/2-1={2.15,2.15,2.15}
                .exclude("ERROR:");
    ...
    public void testTaskC__ConsoleRunner() throws Exception {
        ...
        ;
        run("({{1,2},{8,3}}+{{1,1},{1,1}})*2-{{1,2},{8,3}}\n" +
                "end\n")
                .include("{{3.0, 4.0}, {10.0, 5.0}}")
        ;
    }
    ...
    private static Test_calc_many_expressions run(String in) {
        return run(in, true);
    }

    private static Test_calc_many_expressions run(String in, boolean runMain) {
        ...
        System.out.println("---------------------------------------------");
        return new Test_calc_many_expressions(clName, in, runMain);
    }

    public Test_calc_many_expressions() {
        //Конструктор тестов
    }
    ...
    private Test_calc_many_expressions(String className, String in, boolean runMain) {
    ...
    //проверка вывода
    private Test_calc_many_expressions is(String str) {
        assertTrue("Ожидается такой вывод:\n<---начало---->\n" + str + "<---конец--->",
                stringWriter.toString().equals(str));
        return this;
    }

    private Test_calc_many_expressions include(String str) {
        assertTrue("Строка не найдена: " + str + "\n", stringWriter.toString().contains(str));
        return this;
    }

    private Test_calc_many_expressions exclude(String str) {
