...
public class CmdReport2 implements Cmd {
    @Override
    public Action execute(HttpServletRequest req) throws Exception {
        ...
        String sprOperationsId = "20";

        UserOperationDao userOperationDao = new UserOperationDao();
        List<UserOperation> userOperation20 = userOperationDao.userAndDutiesAndNumberOperation(user.getId(), duties.get(duties.size() - 1).getId(), sprOperationsId);


        for (int i = 0; i < userOperation20.size(); i++) {

            List<UserEntry> userEntryList = new UserEntryDao().getAll("WHERE `userOperationId`=" + userOperation20.get(i).getId());

            for (int j = 0; j < userEntryList.size(); j++) {
                if (userEntryList.get(j).getCurrencyId() == 933) {
                    double sum = userEntryList.get(j).getSum();
                    Report1 report1 = new Report1(userOperation20.get(i), sum);
        ...
        Action.REPORT2.setPATH("/cash/report/");
        return Action.REPORT2;
    }
}
